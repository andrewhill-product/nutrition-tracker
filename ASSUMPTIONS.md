...
- A second optional photo of the packaging nutrition label can be added on the slot picker (the cereal-box flow: bowl photo for the portion, label photo for exact values). Both photos go to the model in one analyse call with an instruction to read the label and scale to the visible portion. A failed label upload degrades gracefully to a single-photo analysis.
